...
class Search {
  ...
  runSolrSearch (
    ...
    const author = filters.author ? `%20AND%20author:${filters.author}` : ''
    ...
    const query = `q=*:*${domain}${docType}${author}${tags}${rating}${timestamp}${userRolesQuery}${categoryQuery}${activeEvent}${fq}${sortQuery}${fields}&start=${start}&rows=${rows}`
  ...
  processFilters (event) {
    ...
    if (_.isString(event.author)) {
      filters.author = event.author
    }
