...
class Search {
  ...
  runSolrSearch (
    ...
    const tags = event.tags && event.tags.length > 0 ? `%20AND%20tags:(${event.tags.map(tag => `"${tag.replace(/ /g, '%20')}"`).join('%20OR%20')})` : ``
    ...
    const query = `q=*:*${domain}${docType}${tags}${timestamp}${userRolesQuery}${categoryQuery}${activeEvent}${fq}${sortQuery}&start=${start}&rows=${rows}`
