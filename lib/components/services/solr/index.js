...
class SolrService {
  async boot (options, callback) {
    ...
    if (!storageClient) {
      callback(new Error('failed to boot solr service: no database client available'))
    }
    ...
    if (!this.createViewSQL) {
      callback(new Error('failed to construct create view SQL'))
    }
  ...
  _executeReindex (type, core, cb) {
    ...
    axios.post(url, form)
      ...
      .then(body => cb(null, JSON.parse(body)))
      .catch(err => cb(err))
  } // _executeReindex
} // class SolrService
