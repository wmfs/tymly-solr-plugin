...
function buildDataImportPost (solrUrl, command, collection) {
  ...
  console.log('IMPORT URL: ', `${url}${collection}/dataimport?_=${uniqueIdentifier}&indent=off&wt=json`)
  return {
    url: `${url}${collection}/dataimport?_=${uniqueIdentifier}&indent=off&wt=json`,
    form: {
      clean,
      command,
      commit: true,
